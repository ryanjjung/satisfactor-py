...
class Availability(object):
    ...
    def __init__(self, tier: int, upgrade: int, mam: bool = False):
        self.tier = tier
    ...
    def to_dict(self) -> dict:
        ...
        return {'tier': self.tier, 'upgrade': self.upgrade}
...
class ComponentError(Exception):
    ...
    def __init__(self, level: ComponentErrorLevel, message: str = '', **kwargs):
...
class Component(Base):
    ...
    def __init__(self, constructed: bool = False, traversed: bool = False, **kwargs):
    ...
    def add_error(self, error: ComponentError):
        """
    ...
    @errors.setter
    def errors(self, value: list[ComponentError]):
        """
...
class Item(Base):
    ...
    def __init__(
        self, conveyance_type: ConveyanceType = ConveyanceType.BELT, stack_size: int = 0, sink_value: int = 0, **kwargs
    ):
        super().__init__(**kwargs)
...
class Ingredient(object):
    ...
    def __init__(self, item: Item, amount: int, rate: float):
        self.item = item
        self.amount = amount
        self.rate = rate
    ...
    def to_dict(self) -> dict:
        ...
        return {'item': self.item.to_dict(), 'amount': self.amount, 'rate': self.rate}
...
class Recipe(Base):
    ...
    def __init__(
        self, building_type: BuildingType, consumes: list[Ingredient] = [], produces: list[Ingredient] = [], **kwargs
    ):
        super().__init__(**kwargs)
    ...
    def to_dict(self) -> dict:
        ...
        base = super().to_dict()
        base.update({'building_type': self.building_type.name, 'consumes': consumes, 'produces': produces})
        return base
...
class Connection(Component):
    ...
    def connected_to(self, skip_conveyances: bool = False):
        """
    ...
    def process(self):
        ...
        if not self.attached_to:
            self.add_error(ComponentError(ComponentErrorLevel.IMPOSSIBLE, 'Connection is not attached to anything'))
...
class Input(Connection):
    ...
    def connect(self, connection: Connection) -> bool:
        ...
        if connection.conveyance_type != self.conveyance_type:
            raise TypeError('Connection target does not support the same conveyance type')

        # The target end must be the opposite direction of this end
        if connection.is_input():
            raise TypeError('Cannot connect two inputs')
...
class Output(Connection):
    ...
    def connect(self, connection: Connection) -> bool:
        ...
        if connection.is_output():
            raise TypeError('Cannot connect two outputs')
    ...
    def process(self):
        ...
        super().process()
        if not self.target:
            self.add_error(ComponentError(ComponentErrorLevel.WARNING, 'Output is not connected to a target'))
        else:
...
class ResourceNode(Component):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(wiki_path=wiki_path, image_path='/8/87/Iron_Ore.png', name=name, **kwargs)
        self.purity = purity
        self.item = item
        self.inputs = []
        self.outputs = [Output(conveyance_type=ConveyanceType.RESOURCE_NODE, attached_to=self)]
    ...
    def to_dict(self) -> dict:
        ...
        base = super().to_dict()
        base.update(
            {'purity': self.purity.name, 'item': self.item.to_dict(), 'outputs': [output.id for output in self.outputs]}
        )
        return base
    ...
    def process(self):
        ...
        if input_ct > 0:
            self.add_error(ComponentError(f'ResourceNodes cannot have any inputs, but this has {input_ct}.'))
        ...
        if output_ct > 1 or output_ct < 1:
            ...
            if not self.outputs[0].target:
                self.add_error(ComponentError(ComponentErrorLevel.WARNING, 'This ResourceNode is not connected'))
            else:
                ...
                if target_bldg.building_type not in (BuildingType.MINER, BuildingType.WATER_EXTRACTOR):
                    ...
                    if target_bldg.recipe is None:
                        self.add_error(ComponentError(ComponentErrorLevel.WARNING, 'The connected Miner has no recipe'))
                    else:
                        target_production = [
                            ingredient.item.programmatic_name() for ingredient in target_bldg.recipe.produces
                        ]
...
class InfiniteSupplyNode(ResourceNode):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(purity=Purity.NORMAL, item=item, wiki_path=wiki_path, name=name, **kwargs)
        self.rate = rate
        self.outputs = [Output(conveyance_type=conveyance_type, attached_to=self)]
    ...
    def can_process(self) -> bool:
        ...
        can_process = True
        if not self.item:
            self.add_error(ComponentError(level=ComponentErrorLevel.IMPOSSIBLE, message="Node doesn't supply an item."))
            self.can_process = False

        if not self.rate:
            self.add_error(ComponentError(level=ComponentErrorLevel.IMPOSSIBLE, message='Node has no rate set.'))
            self.can_process = False

        if not self.outputs[0].target:
            self.add_error(
                ComponentError(level=ComponentErrorLevel.WARNING, message='Node is not connected to a conveyance.')
            )
            self.can_process = False
    ...
    def process(self) -> bool:
        ...
        self.outputs[0].ingredients = [Ingredient(item=self.item, amount=None, rate=rate)]
...
class Building(Component):
    ...
    def can_process(self) -> bool:
        ...
        if self.recipe is None:
            self.add_error(ComponentError(ComponentErrorLevel.WARNING, 'Building has no recipe'))
            success = False

        # Can't process in standby mode
        if self.standby:
            self.add_error(ComponentError(ComponentErrorLevel.WARNING, 'Building is in standby mode'))
            success = False
        ...
        if self.recipe and self.recipe.consumes:
            ...
            if len(self.inputs) < len(self.recipe.consumes):
                self.add_error(
                    ComponentError(ComponentErrorLevel.IMPOSSIBLE, 'Building has fewer inputs than its recipe requires')
                )
                success = False
            ...
            for ingredient in requirements:
                if ingredient.name not in ing_names:
                    self.add_error(
                        ComponentError(
                            ComponentErrorLevel.WARNING, f'Recipe ingredient {ingredient.name} is not available'
                        )
                    )
                    success = False
            ...
            for ingredient in ing_names:
                if ingredient not in requirement_names:
                    self.add_error(
                        ComponentError(
                            ComponentErrorLevel.WARNING, f'Ingredient {ingredient} is not required for the recipe'
                        )
                    )
        ...
        for recipe_ingredient in self.recipe.produces:
            for output in self.outputs:
                ...
                if len(output.ingredients) == 0 and output.conveyance_type == recipe_ingredient.item.conveyance_type:
                    output.ingredients = [
                        Ingredient(recipe_ingredient.item, None, recipe_ingredient.rate * self.clock_rate)
                    ]
                else:
                    success = False
...
class NonProcessingBuilding(Building):
    ...
    def __init__(self, building_type: BuildingType = BuildingType.OTHER, **kwargs):
        super().__init__(building_type=building_type, recipe=None, overclockable=False, inputs=[], outputs=[], **kwargs)
...
class Conveyance(Building):
    ...
    def __init__(self, conveyance_type: ConveyanceType = ConveyanceType.BELT, rate: float = 0, **kwargs):
        super().__init__(
    ...
    def to_dict(self) -> dict:
        ...
        base = super().to_dict()
        base.update({'conveyance_type': self.conveyance_type.name, 'rate': self.rate})
        return base
...
class Storage(Building):
    ...
    def __init__(self, stacks: int, ingredients: list[Ingredient] = None, **kwargs):
        super().__init__(building_type=BuildingType.STORAGE, building_category=BuildingCategory.ORGANIZATION, **kwargs)
        self.rate = None
        self.stacks = stacks
        self.ingredients = ingredients
    ...
    def to_dict(self) -> dict:
        ...
        base = super().to_dict()
        base.update({'stacks': self.stacks})
        return base
