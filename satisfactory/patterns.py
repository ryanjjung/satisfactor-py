...
from satisfactory.conveyances import ConveyorBeltMk1, ConveyorBeltMk2, PipelineMk1
...
def tier_0_cable_factory(purity: Purity = Purity.NORMAL) -> Factory:
    ...
    copperSource = ResourceNode(name=f'{purity.name.title()} Copper Source', purity=purity, item=iCopperOre)

    # Connect it to a miner
    copperMiner = MinerMk1(name='Copper Miner', recipe=rCopperOreMk1)

    copperSource.outputs[0].connect(copperMiner.inputs[0])

    # Connect the miner to a smelter
    copperSmelter = Smelter(name='Copper Smelter', recipe=rCopperIngot)
    convOreToSmelter = copperMiner.connect(copperSmelter, ConveyorBeltMk1)

    # Connect the smelter to a constructor making wire
    wireConstructor = Constructor(name='Wire Constructor', recipe=rWire)
    ...
    cableConstructor = Constructor(name='Cable Constructor', recipe=rCable)
...
def tier_0_concrete_factory(purity: Purity = Purity.NORMAL) -> Factory:
    """
    ...
    limestoneSource = ResourceNode(name=f'{purity.name.title()} Limestone Source', purity=purity, item=iLimestone)

    # Connect a miner to it
    limestoneMiner = MinerMk1(name='Limestone Miner', recipe=rLimestoneMk1)
    limestoneSource.outputs[0].connect(limestoneMiner.inputs[0])

    # Connect it to a constructor producing limestone
    concreteConstructor = Constructor(name='Concrete Constructor', recipe=rConcrete)
...
def tier_1_screw_factory() -> Factory:
    ...
    ironSource = ResourceNode(name='Normal Iron Source', purity=Purity.NORMAL, item=iIronOre)
    ironMiner = MinerMk1(name='Iron Miner', recipe=rIronOreMk1)
    ironSource.outputs[0].connect(ironMiner.inputs[0])
    ...
    ironSmelters = [Smelter(name=f'Iron Smelter #{i + 1}', recipe=rIronIngot) for i in range(2)]
    ...
    ingotSplitters = [ConveyorSplitter(name=f'Iron Ingot Splitter #{i + 1}') for i in range(2)]
    ironSmelters[0].connect(ingotSplitters[0], ConveyorBeltMk1)
    ironSmelters[1].connect(ingotSplitters[1], ConveyorBeltMk1)

    ironRodConstructors = [Constructor(name=f'Iron Rod Constructor #{i + 1}', recipe=rIronRod) for i in range(4)]
    ...
    rodMergers = [ConveyorMerger(name=f'Iron Rod Merger #{i + 1}') for i in range(2)]
    ...
    rodSplitters = [ConveyorSplitter(name=f'Iron Rod Splitter #{i + 1}') for i in range(2)]
    rodMergers[0].connect(rodSplitters[0], ConveyorBeltMk1)
    rodMergers[1].connect(rodSplitters[1], ConveyorBeltMk1)

    screwConstructors = [Constructor(name=f'Screw Constructor #{i + 1}', recipe=rScrew) for i in range(6)]
    ...
    screwStorages = [StorageContainer(name=f'Screw Storage #{i + 1}') for i in range(6)]
    for i in range(6):
    ...
    storageMergers = [ConveyorMerger(name=f'Storage Merger #{i + 1}') for i in range(3)]
    for i in range(3):
...
def tier_2_biofuel_factory() -> Factory:
    ...
    mycelia_storage = StorageContainer(name='Mycelia Storage', ingredients=[Ingredient(iMycelia, None, 1000)])
    leaves_storage = StorageContainer(name='Leaves Storage', ingredients=[Ingredient(iLeaves, None, 1000)])
    wood_storage = StorageContainer(name='Wood Storage', ingredients=[Ingredient(iWood, None, 1000)])

    # Hook them up to Constructors to produce Biomass
    mycelia_constructor = Constructor(name='Biomass from Mycelia Constructor', recipe=rBiomassMycelia)
    leaves_constructor = Constructor(name='Biomass from Leaves Constructor', recipe=rBiomassLeaves)
    wood_constructor = Constructor(name='Biomass from Wood Constructor', recipe=rBiomassWood)
    ...
    biomass_storage = StorageContainer(name='Biomass Storage', ingredients=[Ingredient(iBiomass, None, 1000)])

    # Make four constructors for making solid fuel, hooking one up to each biomass constructor
    biofuel_constructors = [Constructor(name=f'Biofuel Constructor #{i + 1}', recipe=rSolidBiofuel) for i in range(4)]
    ...
    biofuel_mergers = [ConveyorMerger(name=f'Biofuel Merger #{i + 1}') for i in range(3)]
...
def tier_3_steel_beam_factory(ironPurity: Purity = Purity.NORMAL, coalPurity: Purity = Purity.NORMAL) -> Factory:
    """
    ...
    ironSource = ResourceNode(name=f'{ironPurity.name.title()} Iron Source', purity=ironPurity, item=iIronOre)

    coalSource = ResourceNode(name=f'{coalPurity.name.title()} Coal Source', purity=coalPurity, item=iCoal)

    # Connect miners to them
    ironMiner = MinerMk1(name='Iron Miner', recipe=rIronOreMk1)

    coalMiner = MinerMk1(name='Coal Miner', recipe=rCoalMk1)
    ...
    steelBeamConstructor = Constructor(name='Steel Beam Constructor', recipe=rSteelBeam)
    ...
    factory.add([ironSource, coalSource, ironMiner, coalMiner, foundry, steelBeamConstructor, steelBeamStorage])
    return factory


def tier_3_coal_power_factory(purity: Purity = Purity.NORMAL) -> Factory:
    """
    ...
    coalSource = ResourceNode(name='Coal Source', purity=purity, item=iCoal)

    waterSource = ResourceNode(name='Water Source', purity=purity.NORMAL, item=iWater)

    # Attach a miner/extractor to those nodes
    coalMiner = MinerMk1(name='Coal Miner', recipe=rCoalMk1)

    waterExtractor = WaterExtractor(recipe=rWater, clock_rate=1.5)
    ...
    waterSplitter1 = PipelineJunctionCross(name='Water Splitter Tier 1', inputs=1, outputs=2)
    waterSplitter2_1 = PipelineJunctionCross(name='Water Splitter Tier 2 #1', inputs=1, outputs=2)
    waterSplitter2_2 = PipelineJunctionCross(name='Water Splitter Tier 2 #2', inputs=1, outputs=2)
    ...
    coalGenerators = [CoalGenerator(name=f'Coal Generator #{i + 1}', recipe=rCoalPower) for i in range(4)]
